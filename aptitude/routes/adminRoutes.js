  ...
  asyncHandler(async (req, res) => {
    ...
    const [assessments, published, students, submittedCount, inProgressCount, submissions] =
      ...
      AssessmentAttempt.find({ status: "submitted" })
        .populate("student_id", "name collegeEmail")
    ...
    const analytics = submissions.map((attempt) => {
      ...
      return {
        ...
        email: attempt.student_id?.collegeEmail || "",
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const seen = new Set();
    const uniqueErrors = validation.errors.filter((error) => {
      if (error.includes("duplicate question text")) return false;
      return true;
    });

    const uniqueQuestions = validation.questions.filter((question) => {
    ...
    if (uniqueQuestions.length === 0 || uniqueErrors.length > 0) {
      throw badRequest(
        `Generated questions failed validation (${uniqueQuestions.length}/${config.question_count} returned)`,
        uniqueErrors.slice(0, 12),
      );
    }

    const totalMarks = uniqueQuestions.reduce((sum, question) => sum + question.marks, 0);
